...
function submitSurvey() {
	...
	// 세션에서 가져온 userId를 surveyData에 추가
	//surveyData.push({ id: 'userId', response: userId });
    ...
    xhr.onreadystatechange = function() {
        if (xhr.readyState === 4) {
            if (xhr.status === 200) {
                ...
                // 다른 페이지로 리디렉션
                window.location.href = 'http://localhost:8081/SoundTeam/GoRecommendPage';
            } else {
                console.error('서버 오류 발생:', xhr.status, xhr.statusText);
